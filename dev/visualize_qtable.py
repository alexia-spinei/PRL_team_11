...
def generate_all_slices(
    ...
    """Generate policy heatmaps for all 40 slices."""
    ...
    for season in SEASONS:
        for period in PERIODS:
            for day_type in DAY_TYPES:
                ...
                fig.savefig(output_dir / f"policy_{slice_name}.png", dpi=150, bbox_inches="tight")
                plt.close(fig)
                ...
                if visits is not None:
                    ...
                    fig.savefig(output_dir / f"visits_{slice_name}.png", dpi=150, bbox_inches="tight")
                    plt.close(fig)

    print(f"Generated {4 * 5 * 2} slice plots in {output_dir}")
