...
def plot_training_diagnostics(training_metrics: dict, output_dir: Path) -> None:
    ...
    val_hist = training_metrics.get("val_pnl_history", [])  # list of [episode, pnl] or (episode, pnl)
...
def plot_validation_rollout_24h(records: dict, output_dir: Path, H: int = 24, start: int = 40) -> None:
...
def train_q_learning(train_features: dict, config: QConfig, val_features: dict | None = None) -> TrainingResult:
    ...
    for ep in range(config.episodes):
        ...
        if config.track_td_error:
            td_error_history.append(float(np.mean(td_errors_ep)) if td_errors_ep else 0.0)
        ...
        if (
            ...
            val_pnl = greedy_validation_pnl(Q, val_features, storage_bins, price_bins, price_window=config.price_window)
...
def evaluate(
    ...
    """Evaluate Q-table on validation data (greedy policy)."""

    records = None
    if record:
        records = {"price": [], "water": [], "action": [], "reward": [], "pnl": []}
...
def greedy_validation_pnl(
    ...
    cfg = create_dam_config(val_features, storage_bins, price_bins, price_window=price_window)
    env = DamEnvGym(cfg)
    obs, _ = env.reset()
    state = env.discretize(obs)
