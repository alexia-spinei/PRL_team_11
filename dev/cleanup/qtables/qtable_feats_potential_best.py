...
This version tracks and saves the BEST validation Q-table (not final).
"""
...
class TrainingResult:
    ...
    # Best Q-table tracking
    best_Q: np.ndarray | None = None
    best_episode: int = 0
    best_val_pnl: float = float('-inf')
...
def save_results(
    ...
    training_metrics = {
        ...
        "best_val_pnl": result.best_val_pnl,
    }
...
""".strip(),
    ...
    # Save best Q-table (primary output)
    Q_to_save = result.best_Q if result.best_Q is not None else result.Q
    np.savez_compressed(output_dir / "qtable_best.npz", Q=Q_to_save)

    # Also save final Q-table for reference
    np.savez_compressed(output_dir / "qtable_final.npz", Q=result.Q)
    ...
    print(f"  Best Q-table from episode {result.best_episode} (val PnL: {result.best_val_pnl:.2f})")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--eval-every",
        type=int,
        help="Run greedy validation every N episodes (0 disables)",
    )
...
def main():
    ...
    # Use best Q-table for evaluation (if available)
    Q_for_eval = result.best_Q if result.best_Q is not None else result.Q
    print(f"\nBest Q-table from episode {result.best_episode} (val PnL: {result.best_val_pnl:.2f})")

    print("\nEvaluating best Q-table on validation set...")
    ...
    print(f"Validation PnL (best Q-table): {eval_result.total_pnl:.2f}")
